"""
Enhanced Rule Execution Engine - Supports parallel processing optimization, LRU/TTL cache, configuration support
"""
import threading
import time
import os
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                "keys": list(self.cache.keys())[:10]  # Only show first 10
            }
...
class EnhancedRuleEngine:
    ...
    def execute_all(self, context: RuleContext, parallel: Optional[bool] = None) -> Tuple[List[Finding], Dict[str, Any]]:
        ...
        Args:
            ...
            parallel: Whether to execute in parallel (None means use configuration)
    ...
    def _execute_sequential_enhanced(self, context: RuleContext, rules: List[Rule]) -> Tuple[List[Finding], Dict[str, Any]]:
        """Enhanced sequential rule execution"""
        all_findings = []
        execution_stats = {
    ...
    def _execute_parallel_enhanced(self, context: RuleContext, rules: List[Rule]) -> Tuple[List[Finding], Dict[str, Any]]:
        """Enhanced parallel rule execution"""
        all_findings = []
        execution_stats = {
            "total_rules": len(rules),
            "execution_time": 0,
    ...
    def _execute_rule_with_timeout(self, rule: Rule, context: RuleContext) -> Tuple[List[Finding], bool]:
        """Rule execution with timeout control"""
    ...
    def _execute_rule_enhanced(self, rule: Rule, context: RuleContext) -> Tuple[List[Finding], bool]:
        ...
        try:
            ...
            # Log error but don't interrupt entire scan
    ...
    def _get_cache_key(self, rule: Rule, context: RuleContext) -> str:
        """Generate cache key"""
        # Include rule ID, file hash, and rule configuration hash
    ...
    def _count_findings_by_severity(self, findings: List[Finding]) -> Dict[str, int]:
        """Count number of issues by severity level"""
    ...
    def execute_by_category(self, context: RuleContext, category: str, parallel: Optional[bool] = None) -> Tuple[List[Finding], Dict[str, Any]]:
        ...
        try:
            ...
            # If cannot convert, return empty result
    ...
    def warmup_cache(self, contexts: List[RuleContext]):
        ...
        """
        if not self.cache:
            logger.info("Cache not enabled, skipping warmup")
            return
        
        logger.info(f"Starting cache warmup, {len(contexts)} contexts")
        
        rules = self.registry.get_all_rules(enabled_only=True)
        if not rules:
            logger.info("No enabled rules, skipping warmup")
            return
        ...
        logger.info(f"Cache warmup completed: hits {cache_hits}, misses {cache_misses}, time {elapsed_time:.2f} seconds")
